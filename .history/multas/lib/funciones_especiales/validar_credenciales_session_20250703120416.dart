...
class AuthService {
  ...
  Future<bool> validarCredenciales({
    ...
    try {
      ...
      final dbFechaHora = await dbHelper.getConfiguracion('DBFECHAHORA');

      // 2. Construir URL según el formato WS_LOGIN
      final url = Uri.parse(
        '$baseUrl/login.json?clave=$matricula&contrasena=$password&tipo=Nueva&dbFechaHora=$dbFechaHora',
      );
      ...
      return await _validarCredencialesLocales(matricula, password);
    }
  }
