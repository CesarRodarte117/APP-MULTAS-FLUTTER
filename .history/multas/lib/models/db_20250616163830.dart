...
import 'multas.dart';
export 'db.dart';
import 'package:postgres/postgres.dart'; // Nueva dependencia
...
''');
  ...
  // CALLES
    // Método para obtener calles de PostgreSQL externa
  Future<List<Calles>> fetchCallesFromExternalDB() async {
    final connection = PostgreSQLConnection(
      'db.tarkus.mx',     // Host
      5432,               // Puerto
      'mpiojuarez_multas_pruebas', // Database
      username: 'tarkus', // Usuario
      password: 'tarkus', // Contraseña
    );
  ...
  // Método para sincronizar calles
  Future<void> syncCalles() async {
    final db = await database;

    // Obtener calles de la DB externa
    final externalCalles = await fetchCallesFromExternalDB();

    // Limpiar tabla local
    await db.delete('calles');

    // Insertar nuevas calles
    final batch = db.batch();
    for (var calle in externalCalles) {
      batch.insert(
        'calles',
        {
          'id': calle.id,
