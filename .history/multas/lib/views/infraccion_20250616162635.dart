...
class _InfraccionState extends State<Infraccion> {
  int _currentStep = 0;
  List<Calles> _calles = []; // Movido al State
  bool _loadingCalles = false; // Movido al State
  final TextEditingController _calleController =
  ...
  void initState() {
    ...
    _loadCalles(); // Llamar al cargar calles al iniciar
  }

  @override
  void dispose() {
    _calleController.dispose(); // Importante para liberar recursos
    super.dispose();
  }

  Future<void> _loadCalles() async {
    if (_calles.isNotEmpty) return; // No recargar si ya hay datos
    ...
    try {
      final connection = PostgreSQLConnection(
        ...
        username: 'tu_usuario', // Reemplaza con tu usuario real
        password: 'tarkus',
      );
